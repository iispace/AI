# Rpi 5 <--> Arduino Uno UART 통신 (양방향 통신과 Thread 간 데이터 접근 테스트)
  ...
  - main thread에서는 카메라가 켜져 있는 동안 1초에 한번씩 제어 데이터 값을 갱신하고, 아두이노에서 수신한 센서 값을 화면에 출력한다.
  - SubThread로 동작하는 serial_thread()에서는 제어 데이터를 아두이노에 보내고, 아두이노에서 보내온 센서 값을 받는다.
  - Rpi 5로부터 제어 신호를 아무것도 받지 못하면 Arudino는 모터를 정지한다.
