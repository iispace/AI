...
|구분|위 논문의 내용|나의 연구 방향|
|:-|:-|:-|
|차별점|출발지점과 위치가 다른 목표 지점이 사전에 설정된 시나리오|목표 지점에 출발 지점과 같은 시나리오<b>(예) 출발한 시간으로부터 10분간 주변 정찰 후 출발지로 복귀|
|차별점|LiDAR 센서와 카메라 사용|RGB 단안 카메라만 사용|
|차별점|시뮬레이션 기반 연구|실제 환경 기반 연구|
|유사점|DQN적용|DQN 및 DQN + $\alpha$적용|
