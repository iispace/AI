...
|용어|설명|
|:-|:-|
|Holonomic robot|드론과 같이 원하는 방향으로 즉시 이동이 가능(전,후,좌,우,대각) 로봇. 이동 방향의 제약이 적어 "즉각적인 자유도"가 큼|
|Non-holomonic robot|대각선으로는 즉시 이동하지 못하는 자동차형 로봇과 같이 특정 방향으로는 직접 이동이 불가능한 로봇. 전진/후진과 좌/우 회전을 하나의 "경로"로 조합해야만 대각선 방향으로의 이동이 가능한 로봇|
...
- 수변과 agent 간의 관계는 MDP로 표현 (surroundings, agent actions, and rewards)
